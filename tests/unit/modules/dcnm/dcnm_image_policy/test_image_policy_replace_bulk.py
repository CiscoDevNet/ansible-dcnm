...
from ansible_collections.cisco.dcnm.tests.unit.modules.dcnm.dcnm_image_policy.utils import (
    MockImagePolicies, does_not_raise, image_policy_replace_bulk_fixture,
    payloads_image_policy_replace_bulk, responses_image_policy_replace_bulk,
...
def test_image_policy_replace_bulk_00031(
    ...
    Summary
    Simulate a request to replace a policy that does not exist on the controller
...
def test_image_policy_replace_bulk_00035(
    ...
    PATCH_DCNM_SEND += "module_utils.image_policy.replace.dcnm_send"
    ...
    assert instance.response_current == responses_image_policy_replace_bulk(key)
    ...
    assert instance.diff_ok[0]["policyDescr"] == "KR5M replaced"
    assert instance.diff_ok[1]["policyName"] == "NR3F"
    assert instance.diff_ok[1]["policyDescr"] == "NR3F replaced"
    assert instance.response_nok == []
    assert instance.result_nok == []
    assert instance.diff_nok == []
...
def test_image_policy_replace_bulk_00036(
    ...
    Summary
    Simulate a 500 response from the controller during policy replace.
...
def test_image_policy_replace_bulk_00037(
    ...
    from the controller during policy replace.  In this case, the following holds true:
    - The bad response is recorded in response_nok, result_nok, and diff_nok.
    ...
    key_payloads = "test_image_policy_replace_bulk_00037c"

    with does_not_raise():
        instance = image_policy_replace_bulk

    monkeypatch.setattr(instance, "diff_ok", payloads_image_policy_replace_bulk(key_ok))
    monkeypatch.setattr(
        instance, "diff_nok", payloads_image_policy_replace_bulk(key_nok)
    )
    monkeypatch.setattr(
        instance,
        "_payloads_to_commit",
        payloads_image_policy_replace_bulk(key_payloads),
    )
    monkeypatch.setattr(
        instance, "response_ok", responses_image_policy_replace_bulk(key_ok)
    )
    monkeypatch.setattr(
        instance, "response_nok", responses_image_policy_replace_bulk(key_nok)
    )
    monkeypatch.setattr(
        instance, "result_ok", results_image_policy_replace_bulk(key_ok)
    )
    monkeypatch.setattr(
        instance, "result_nok", results_image_policy_replace_bulk(key_nok)
    )

    match = r"ImagePolicyReplaceBulk._process_responses: Bad response\(s\) "
    match += r"during image policy bulk replace\."
    with pytest.raises(AnsibleFailJson, match=match):
    ...
    diff = payloads_image_policy_replace_bulk(key_ok)
    diff[0]["action"] = "replace"
    assert instance.diff[0] == diff[0]
    assert instance.result == results_image_policy_replace_bulk(key_ok)
    assert instance.response == responses_image_policy_replace_bulk(key_ok)
