...
"""
Initiate a fabric-group config-save operation on the controller
"""
...
from ..common.api.onemanage.endpoints import EpOneManageFabricConfigSave
from ..common.conversion import ConversionUtils
from ..common.rest_send_v2 import RestSend
from ..common.results_v2 import Results


class FabricGroupConfigSave:
    """
    # Initiate a fabric-group config-save operation on the controller.

    -   Raise ``ValueError`` for any caller errors, e.g. required properties
        not being set before calling FabricGroupConfigSave().commit().
    -   Update FabricGroupConfigSave().results to reflect success/failure of
    ...
    config_save = FabricGroupConfigSave()
    ...
    def __init__(self):
        ...
        self.cannot_save_fabric_reason: str = ""
        self.config_save_failed: bool = False
        self.fabric_can_be_saved: bool = False

        self.config_save_result: dict[str, bool] = {}

        self.conversion: ConversionUtils = ConversionUtils()
        self.ep_config_save: EpOneManageFabricConfigSave = EpOneManageFabricConfigSave()
    ...
    def _can_fabric_be_saved(self) -> None:
        ...
        if not self.payload:
            msg = f"Fabric {self.fabric_name} payload is empty. "
        ...
        if not isinstance(self.payload, dict):
            msg = f"Fabric {self.fabric_name} payload is not a dictionary. "
            msg += "Skipping config-save."
            self.log.debug(msg)
            self.cannot_save_fabric_reason = msg
            self.config_save_failed = False
            self.fabric_can_be_saved = False
            return
        if self.payload.get("FABRIC_NAME", None) is None:
            msg = f"Fabric {self.fabric_name} payload is missing FABRIC_NAME. "
            msg += "Skipping config-save."
            self.log.debug(msg)
            self.cannot_save_fabric_reason = msg
            self.config_save_failed = False
            self.fabric_can_be_saved = False
            return
        if self.payload["FABRIC_NAME"] != self.fabric_name:
            msg = f"Fabric {self.fabric_name} payload FABRIC_NAME "
            msg += f"({self.payload['FABRIC_NAME']}) does not match. "
            msg += "Skipping config-save."
            self.log.debug(msg)
            self.cannot_save_fabric_reason = msg
            self.config_save_failed = False
