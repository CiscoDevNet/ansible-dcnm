...
class ControllerVersion:
    ...
    def refresh(self):
        ...
        if "DATA" not in self.rest_send.response_current:
            msg = f"{self.class_name}.{method_name}: "
            msg += "response does not contain DATA key. Controller response: "
            msg += f"{self.rest_send.response_current}"
            raise ValueError(msg)
        self._response_data = self.rest_send.response_current["DATA"]
