...
class DcnmVrf11:
    ...
    dcnm_vrf module implementation for DCNM Version 11
    """
    ...
    def format_diff(self) -> None:
        ...
        for want_d in diff_create:
            ...
            try:
                vrf_controller_to_playbook = VrfControllerToPlaybookV11Model(**json_to_dict)
            ...
            msg = f"found_c: POST_UPDATE_v11: {json.dumps(found_c, indent=4, sort_keys=True)}"
            self.log.debug(msg)
    ...
    def validate_vrf_config(self) -> None:
        ...
        Validate self.config against VrfPlaybookModelV11 and update
        ...
        for vrf_config in self.config:
            try:
                ...
                config = VrfPlaybookModelV11(**vrf_config)
                msg = f"config.model_dump_json(): {config.model_dump_json()}"
                self.log.debug(msg)
                self.log.debug("Calling VrfPlaybookModelV11 DONE")
            except pydantic.ValidationError as error:
