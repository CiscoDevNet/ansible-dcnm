        ...
        for cfg in config:
            ...
            if cfg.get("switch", None) is not None:
                for sw in cfg["switch"]:
                    if isinstance(sw, list):
                        msg = " !!! Switches included in playbook profiles must be individual items, but given switch element = {} is a list ".format(
                            sw
                        )
                        warnings.append(msg)
                        flattened = True
            ...
            for k in keys:
                ...
                if ("profile" in k) and (k != "profile"):
                    msg = " !!! Profile name included in playbook tasks must be 'profile', but given profile name = '{}' ".format(
                        k
                    )
                    warnings.append(msg)
                    pop_key = k
