    ...
    suboptions:
      ...
      - VXLAN_EVPN_FABRIC_PARAMETERS:
          description:
            ...
            - Fabric for a VXLAN EVPN deployment with Nexus 9000 and 3000
              switches.
            - The following parameters are specific to VXLAN EVPN fabrics.
            - The indentation of these parameters is meant only to logically
              group them.
            - They should be at the same YAML level as FABRIC_TYPE and
              FABRIC_NAME.
          suboptions:
            - AAA_REMOTE_IP_ENABLED:
                default: 0
                description:
                  - Enable only, when IP Authorization is enabled in the AAA
                    Server
            ...
            - ADVERTISE_PIP_BGP:
                default: 0
                description:
                  - For Primary VTEP IP Advertisement As Next-Hop Of Prefix
                    Routes
            ...
            - ADVERTISE_PIP_ON_BORDER:
                default: 1
                description:
                  - Enable advertise-pip on vPC borders and border gateways
                    only. Applicable only when vPC advertise-pip is not enabled
            ...
            - ANYCAST_BGW_ADVERTISE_PIP:
                ...
                description:
                  - To advertise Anycast Border Gateway PIP as VTEP. Effective
                    on MSD fabric Recalculate Config
            ...
            - AUTO_SYMMETRIC_DEFAULT_VRF:
                default: 0
                description:
                  - Whether to auto generate Default VRF interface and BGP
                    peering configuration on managed neighbor devices. If set,
                    auto created VRF Lite IFC links will have Auto Deploy
                    Default VRF for Peer enabled.
            ...
            - AUTO_SYMMETRIC_VRF_LITE:
                default: 0
                description:
                  - Whether to auto generate VRF LITE sub-interface and BGP
                    peering configuration on managed neighbor devices. If set,
                    auto created VRF Lite IFC links will have Auto Deploy for
                    Peer enabled.
            ...
            - AUTO_UNIQUE_VRF_LITE_IP_PREFIX:
                ...
                description:
                  - When enabled, IP prefix allocated to the VRF LITE IFC is not
                    reused on VRF extension over VRF LITE IFC. Instead, unique
                    IP Subnet is allocated for each VRF extension over VRF LITE
                    IFC.
            ...
            - AUTO_VRFLITE_IFC_DEFAULT_VRF:
                ...
                description:
                  - Whether to auto generate Default VRF interface and BGP
                    peering configuration on VRF LITE IFC auto deployment. If
                    set, auto created VRF Lite IFC links will have Auto Deploy
                    Default VRF enabled.
            ...
            - BGP_AS:
                default: ""
                description:
                  - 1-4294967295 | 1-65535.0-65535 It is a good practice to have
                    a unique ASN for each Fabric.
            ...
            - DEFAULT_QUEUING_POLICY_CLOUDSCALE:
                ...
                description:
                  - Queuing Policy for all 92xx, -EX, -FX, -FX2, -FX3, -GX
                    series switches in the fabric
            ...
            - DEFAULT_VRF_REDIS_BGP_RMAP:
                ...
                description:
                  - Route Map used to redistribute BGP routes to IGP in default
                    vrf in auto created VRF Lite IFC links
            ...
            - DNS_SERVER_VRF:
                default: ""
                description:
                  - One VRF for all DNS servers or a comma separated list of
                    VRFs, one per DNS server
            ...
            - ENABLE_AAA:
                default: 0
                description:
                  - Include AAA configs from Manageability tab during device
                    bootup
            ...
            - ENABLE_NGOAM:
                default: 1
                description:
                  - Enable the Next Generation (NG) OAM feature for all switches
                    in the fabric to aid in trouble-shooting VXLAN EVPN fabrics
            ...
            - ENABLE_PBR:
                default: 0
                description:
                  - When ESR option is ePBR, enable ePBR will enable pbr, sla
                    sender and epbr features on the switch
            ...
            - IBGP_PEER_TEMPLATE:
                default: ""
                description:
                  - Speficies the iBGP Peer-Template config used for RR and
                    spines with border role.
            ...
            - IBGP_PEER_TEMPLATE_LEAF:
                ...
                description:
                  - Specifies the config used for leaf, border or border
                    gateway. If this field is empty, the peer template defined
                    in iBGP Peer-Template Config is used on all BGP enabled
                    devices (RRs,leafs, border or border gateway roles.
            ...
            - ISIS_OVERLOAD_ENABLE:
                default: 1
                description:
                  - When enabled, set the overload bit for an elapsed time after
                    a reload
            ...
            - L3VNI_MCAST_GROUP:
                default: 239.1.1.0
                description:
                  - Default Underlay Multicast group IP assigned for every
                    overlay VRF.
            ...
            - MULTICAST_GROUP_SUBNET:
                ...
                description:
                  - Multicast pool prefix between 8 to 30. A multicast group IP
                    from this pool is used for BUM traffic for each overlay
                    network.
            ...
            - NTP_SERVER_VRF:
                default: ""
                description:
                  - One VRF for all NTP servers or a comma separated list of
                    VRFs, one per NTP server
            ...
            - PER_VRF_LOOPBACK_IP_RANGE:
                default: 10.5.0.0/22
                description:
                  - Prefix pool to assign IP addresses to loopbacks on VTEPs on
                    a per VRF basis
            ...
            - SEED_SWITCH_CORE_INTERFACES:
                default: ""
                description:
                  - Core-facing Interface list on Seed Switch (e.g.
                    e1/1-30,e1/32)
            ...
            - SYSLOG_SEV:
                default: ""
                description:
                  - "Comma separated list of Syslog severity values, one per
                    Syslog server "
                max: 7
                min: 0
            ...
            - VRF_LITE_AUTOCONFIG:
                ...
                description:
                  - VRF Lite Inter-Fabric Connection Deployment Options. If
                    Back2Back&ToExternal is selected, VRF Lite IFCs are auto
                    created between border devices of two Easy Fabrics, and
                    between border devices in Easy Fabric and edge routers in
                    External Fabric. The IP address is taken from the VRF Lite
                    Subnet IP Range pool.
            ...
            - enableRealTimeBackup:
                default: ""
                description:
                  - Backup hourly only if there is any config deployment since
                    last backup
      ...
      - VXLAN_EVPN_FABRIC_MSD_PARAMETERS:
          ...
          suboptions:
            ...
            - ENABLE_RS_REDIST_DIRECT:
                ...
                description:
                  - For auto-created Multi-Site overlay IFCs in Route Servers.
                    Applicable only when Multi-Site Overlay IFC Deployment
                    Method is Centralized_To_Route_Server.
            ...
            - RP_SERVER_IP:
                default: ""
                description:
                  - Multi-Site Route-Server peer list (typically loopback IP
                    address on Route-Server for Multi-Site EVPN peering with
                    BGWs), e.g. 128.89.0.1, 128.89.0.2
      ...
      - LAN_CLASSIC_PARAMETERS:
          description:
            ...
            - Fabric to manage a legacy Classic LAN deployment with Nexus
              switches.
            - The indentation of these parameters is meant only to logically
              group them.
            - They should be at the same YAML level as FABRIC_TYPE and
          ...
          suboptions:
            ...
            - FABRIC_FREEFORM:
                default: ""
                description:
                  - Additional supported CLIs for all same OS (e.g. all NxOS
                    etc) switches
            ...
            - INBAND_ENABLE:
                default: 0
                description:
                  - "Enable POAP over Inband Interface (Pre-req: Inband Mgmt
                    Knob should be Enabled)"
            ...
            - IS_READ_ONLY:
                default: 1
                description:
                  - If enabled, fabric is only monitored. No configuration will
                    be deployed
