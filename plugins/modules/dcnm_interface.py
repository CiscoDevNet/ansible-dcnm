        ...
        for have in self.have_all:
            ...
            if (
                ...
                if (
                    ...
                    # vPC interfaces should NEVER be deleted through interface override.
                    # vPC interfaces are auto-generated by NDFC when:
                    # - TOR uplinks are created (tor/vPC pairing)
                    # - vPC pairs are configured (vPC domain)
                    #
                    # These vPC interfaces (vPC1, vPC2, etc.) represent the vPC bundle and
                    # are tightly coupled with member port-channel configurations. Deleting
                    # a vPC interface while member ports are still associated causes errors:
                    # "Peer-1/2 member port is a member of vPCx, remove port from vPCx first"
                    #
                    # vPC interfaces should only be managed through:
                    # - dcnm_vpc_pair module (for vPC domain management)
                    # - TOR pairing API (for TOR uplink vPCs)
                    # - Direct NDFC UI/API for vPC-specific operations
                    #
                    # Skip ALL vPC interfaces unconditionally during override.
