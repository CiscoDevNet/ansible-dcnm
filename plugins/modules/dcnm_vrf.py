...
class DcnmVrf:
                ...
                for have in have_a:
                    if want["serialNumber"] == have["serialNumber"]:
                        if (
                            ...
                            want_e = ast.literal_eval(
                                want_ext_values["VRF_LITE_CONN"]
                            )
                            have_e = ast.literal_eval(
                                have_ext_values["VRF_LITE_CONN"]
                            )
    ...
    def update_attach_params(self, attach, vrf_name, deploy, vlanId):
        ...
        if role.lower() == "spine" or role.lower() == "super spine":
            msg = "VRFs cannot be attached to switch {} with role {}".format(
        ...
        if attach["vrf_lite"]:
            ...
            if not r:
                msg = "VRF LITE cannot be attached to switch {} with role {}".format(
                    attach["ip_address"], role
                )
                self.module.fail_json(msg=msg)
            ...
            for a_l in at_lite:
                if (
                    ...
                    vrflite_con["VRF_LITE_CONN"][0]["IF_NAME"] = a_l[
                        "interface"
                    ]
                    vrflite_con["VRF_LITE_CONN"][0]["DOT1Q_ID"] = str(
                        a_l["dot1q"]
                    )
                    vrflite_con["VRF_LITE_CONN"][0]["IP_MASK"] = a_l[
                        "ipv4_addr"
                    ]
                    ...
                    vrflite_con["VRF_LITE_CONN"][0]["IPV6_MASK"] = a_l[
                        "ipv6_addr"
                    ]
                    ...
                    vrflite_con["VRF_LITE_CONN"][0]["PEER_VRF_NAME"] = a_l[
                        "peer_vrf"
                    ]
                    vrflite_con["VRF_LITE_CONN"][0][
        ...
        if self.vrf_ext:
            attach.update(
                {"extensionValues": json.dumps(ext_values).replace(" ", "")}
            )
            attach.update(
                {
    ...
    def diff_for_create(self, want, have):
        ...
        if vlanId_want != "0":
            ...
            if want["vrfId"] is not None and have["vrfId"] != want["vrfId"]:
                self.module.fail_json(
                    msg="vrf_id for vrf:{} cant be updated to a different value".format(
                        want["vrfName"]
                    )
                )
            elif (
    ...
    def get_have(self):
        ...
        path = "/rest/top-down/fabrics/{}/vrfs/attachments?vrf-names={}".format(
            self.fabric, curr_vrfs[:-1]
        )
    ...
    def get_want(self):
        ...
        for vrf in self.validated:
            ...
            for attach in vrf["attach"]:
                deploy = (
                    vrf_deploy if "deploy" not in attach else attach["deploy"]
                )
                vrfs.append(
                    self.update_attach_params(
                        attach, vrf["vrf_name"], deploy, vlanId
                    )
                )
            ...
            if not vrf_found:
                ...
                if vrf_id is None:
                    ...
                    while True or attempt < 10:
                        attempt += 1
                        path = "/rest/managed-pool/fabrics/{}/partitions/ids".format(
                            self.fabric
                        )
                        ...
                        if missing_fabric or not_ok:
                            ...
                            msg2 = (
                                "Unable to generate vrfId for vrf: {} "
                                "under fabric: {}".format(
                    ...
                    if not vrf_id:
                        self.module.fail_json(
                            msg="Unable to generate vrfId for vrf: {} "
                            "under fabric: {}".format(
                                want_c["vrfName"], self.fabric
                            )
                        )

                    create_path = "/rest/top-down/fabrics/{}/vrfs".format(
                        self.fabric
                    )
    ...
    def format_diff(self):
        ...
        diff_undeploy = (
            self.diff_undeploy["vrfNames"].split(",")
        ...
        for want_d in diff_create:
            ...
            found_c.update(
                {"vrf_extension_template": found_c["vrfExtensionTemplate"]}
            )
            del found_c["source"]
            found_c.update({"source": src})
            found_c.update(
                {"service_vrf_template": found_c["serviceVrfTemplate"]}
            )
    ...
    def get_diff_query(self):
        ...
        if (
            ...
            self.module.fail_json(
                msg="Fabric {} not present on DCNM".format(self.fabric)
            )
            return
                    ...
                    if want_c["vrfName"] == vrf["vrfName"]:
                        ...
                        if want_c.get("vlanId", None) is not None:
                            if str(want_vlanId) != vrf_vlanId:
                                continue
                    if (
                        want_c["vrfName"] == vrf["vrfName"]
                        and want_c["vrfId"] == vrf["vrfId"]
                        and str(want_vlanId) == vrf_vlanId
                    ):
                        ...
                        for vrf_attach in vrf_attach_objects["DATA"]:
                            if want_c["vrfName"] == vrf_attach["vrfName"]:
                                ...
                                for attach in attach_list:
                                    ...
                                    lite_objects = dcnm_send(
                                        self.module, method, path
                                    )
                                    if not lite_objects.get("DATA"):
                                        return
                                    item["attach"].append(
                                        lite_objects.get("DATA")[0]
                                    )
                                query.append(item)
    ...
    def push_to_remote(self, is_rollback=False):
        ...
        if self.diff_create_update:
            for vrf in self.diff_create_update:
                ...
                resp = dcnm_send(
                    self.module, method, update_path, json.dumps(vrf)
                )
                self.result["response"].append(resp)
                fail, self.result["changed"] = self.handle_response(
                    resp, "create"
        ...
        if del_failure:
            self.result["response"].append(
                "Deletion of vrfs {} has failed".format(del_failure[:-1])
            )
                ...
                for v_a in d_a["lanAttachList"]:
                    if v_a.get("vrf_lite"):
                        ...
                        if ext_values is None:
                            msg = "There is no VRF LITE capable interface on this witch {}".format(
                                v_a["ip_address"]
                            )
                            self.module.fail_json(msg=msg)
