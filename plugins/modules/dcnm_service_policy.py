...
class DcnmServicePolicy:
    ...
    def dcnm_sp_validate_sp_input(self, cfg):
        ...
        sp_policy_spec = dict(
            proto=dict(
                required=True, type="str", choices=["ip", "icmp", "tcp", "udp"]
            ),
    ...
    def dcnm_sp_get_service_node_type(self, sp):
        ...
        path = (
            ...
            + self.module.params["attached_fabric"]
            + "/service-nodes/"
            + sp["node_name"]
        )
    ...
    def dcnm_sp_get_sp_payload(self, sp):
        ...
        sp_payload["fabricName"] = self.module.params["attached_fabric"]
        ...
        if sp["policy"][0]["rev_route_map_num"] != 0:
            if sp["reverse"]:
                sp_payload["nvPairs"][
                    "ROUTE_MAP_MATCH_SEQUENCE_NUMBER_REVERSE"
                ] = str(sp["policy"][0]["rev_route_map_num"])
    ...
    def dcnm_sp_update_policy_information(self, want, have, cfg):
        ...
        if cfg["policy"].get("action", None) is None:
            want["nvPairs"]["ROUTE_MAP_ACTION"] = have["nvPairs"][
                "ROUTE_MAP_ACTION"
            ]
        if cfg["policy"].get("next_hop_option", None) is None:
            want["nvPairs"]["NEXT_HOP_OPTION"] = have["nvPairs"][
                "NEXT_HOP_OPTION"
            ]
        ...
        for sp in self.want:
            ...
            match_have = [
                ...
                if (
                    ...
                    and (
                        sp["attachedFabricName"] == have["attachedFabricName"]
                    )
                )
            ]
            if match_have == []:
            ...
            self.dcnm_sp_update_policy_information(
                sp, match_have[0], match_cfg[0]
            )
    ...
    def dcnm_sp_compare_policy_info(self, want, have):
        ...
        if (
            str(want["reverseEnabled"]).lower()
            != str(have["reverseEnabled"]).lower()
        ):
        ...
        if self.sp_info == []:
            ...
            for snode in serv_nodes:
                ...
                if snode.get("name") != "":
                    ...
                    sps = self.dcnm_sp_get_sp_info_with_service_node(
                        snode["node_name"]
                    )
                    ...
                    if snode.get("rp_name") != "":
                        match_sps = [
                            sp
                            for sp in sps
                            if (sp["peeringName"] == snode["rp_name"])
                        ]
                    else:
                        match_sps = sps
    ...
    def dcnm_sp_get_diff_query(self):
        ...
        for sp in self.sp_info:
            ...
            if sp["name"] != "None":
                ...
                resp = self.dcnm_sp_get_sp_info_with_service_node(
                    sp["node_name"]
                )
        ...
        for sp in sp_list:
            match_want = [
                ...
                if (
                    ...
                    and (
                        sp["attachedFabricName"] == want["attachedFabricName"]
                    )
                )
            ]
            if match_want == []:
    ...
    def dcnm_sp_send_message_to_dcnm(self):
        ...
        if self.diff_delete:
            ...
            for sp in self.diff_delete:
                delete_pol_info = self.dcnm_sp_combine_policies(
                    sp, delete_pol_info
                )
        else:
            delete_pol_info = {}
        ...
        if self.diff_deploy:
            ...
            for sp in self.diff_deploy:
                deploy_pol_info = self.dcnm_sp_combine_policies(
                    sp, deploy_pol_info
                )
        else:
            deploy_pol_info = {}
...
def main():
    ...
    element_spec = dict(
        ...
        attached_fabric=dict(required=True, type="str"),
    ...
    dcnm_sp.debug = module.params["debug"]
