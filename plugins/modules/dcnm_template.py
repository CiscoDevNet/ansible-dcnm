...
class DcnmTemplate:
    ...
    def dcnm_template_validate_input (self):
        ...
        template_info, invalid_params = validate_list_of_dicts(self.config, template_spec)
        if invalid_params:
    ...
    def dcnm_template_compare_template (self, template):
        ...
        for have in self.have:
            ...
            if (template['template_name'] == have ['name']):
                ...
                match_pb = [t for t in self.pb_input if template['template_name'] == t['name']][0]

                if (match_pb):
                    if ((desc := match_pb.get('description', None)) == None):
                    ...
                    if ((tags := match_pb.get('tags', None)) == None):
                    ...
                    if (update_content == True):
                        template['content'] \
                             = self.dcnm_template_build_content(match_pb['content'], 
    ...
    def dcnm_template_get_have_all (self):
        ...
        if (resp and (resp['RETURN_CODE'] == 200) and (resp['MESSAGE'] == 'OK') and resp['DATA']):
            for p in resp['DATA']:
                if (p['templateName'] in tlist):
                    ...
                    policies[p['templateName']][p['policyId']]['fabricName']   = p['fabricName']
                    policies[p['templateName']][p['policyId']]['serialNumber'] = p['serialNumber']
    ...
    def dcnm_template_delete_template (self, del_payload):
        ...
        if ('Template deletion successful' in resp['DATA']):
            ...
            resp['DATA'] = resp['DATA'].replace('Invalid JSON response: ', '')
            resp['DATA'] = resp['DATA'].replace('\"notDeletdTemplate\"', 'Templates in use, not deleted')
    ...
    def dcnm_template_get_have(self):
        ...
        for template in self.want:
            ...
            template_payload = self.dcnm_template_get_template_info_from_dcnm(path, name)
    ...
    def dcnm_template_get_diff_query(self):
        ...
        for template in self.template_info:
            ...
            template_payload = self.dcnm_template_get_template_info_from_dcnm(path, template['name'])
