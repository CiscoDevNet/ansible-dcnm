...
version_added: "0.9.0"
description:
    - "DCNM Ansible Module for Creating, Deleting, Querying and Modifying
    - policies
  ...
  config:
    ...
    suboptions:
      ...
      switch:
        ...
        elements: dict
        suboptions:
          ip: IP address of the switch where the policy is to be deployed
          type: ipv4
          required: true
...
class DcnmPolicy:
    ...
    def __init__(self, module):
        ...
        self.deploy_payload = []
        self.fd             = None
        self.changed_dict   = [{'merged' : [], 'deleted' : [], 'deploy' : [], 'query' : [], 'skipped' : []}]
    ...
    def dcnm_policy_validate_input (self):
        ...
        for cfg in self.config:
            ...
            if invalid_params:
                mesg = 'Invalid parameters in playbook: while processing policy "{}", Error: {}'.format(cfg['name'], invalid_params)
    ...
    def dcnm_get_policy_payload_with_template_name (self, pelem, sw):
        ...
        if (pelem.get("Vars", None) != None):

            # Given policy has arguments. Add these to the payload

            for var in pelem["Vars"]:
                policy_payload["nvPairs"][var] = pelem["Vars"][var]
        ...
        for pelem in self.policy_info:
            for sw in pelem['switch']:
                if ('POLICY-' in pelem['name']):
                    ...
                    # updating a policy
                    policy_payload = self.dcnm_get_policy_payload_with_policy_id (pelem, sw)
                else:
                    # Template name is given. Get the 'POST' payload which is used for creating
                    # new policies
                    policy_payload = self.dcnm_get_policy_payload_with_template_name (pelem, sw)
                if (policy_payload):
    ...
    def dcnm_policy_get_have(self):
        ...
        match_pol = [pl for pl in plist for wp in self.want if (pl['templateName'] == wp['templateName'])]
            ...
            if (have[key] == policy[key]):
                ...
                if (have.get('description', None) == policy['description']):
                    if (have.get('priority', None) == policy['priority']):
                        if (have.get('serialNumber', None) == policy['serialNumber']):
                            if (self.dcnm_policy_compare_nvpairs (policy.get('nvPairs', None),\
                                                                  have.get('nvPairs', None)) == 'DCNM_POLICY_MATCH'):
    ...
    def dcnm_policy_get_diff_merge(self):
        ...
        for policy in self.want:
            ...
            if (rc == 'DCNM_POLICY_ADD_NEW'):
                ...
                if ((policy not in self.diff_create) or (policy['create_additional_policy'] == True)):
                ...
                if (policy.get ('policyId', None) != None):
                    ...
                    self.changed_dict[0]['skipped'].append({'Template' : policy['templateName'],\
            ...
            if (self.deploy == True):
                ...
                if ((policy_id != None) and (policy_id not in self.deploy_payload)):
    ...
    def dcnm_policy_get_diff_deleted(self):
        ...
        match_pol = [pl for pl in plist for wp in self.want if ((pl['templateName'] == wp['templateName']) or (pl['policyId'] == wp['templateName']))]
        ...
        for pol in match_pol:
            ...
            self.changed_dict[0]['deleted'].append({'policy' : pol['policyId'], 'templateName' : pol['templateName']})
    ...
    def dcnm_policy_get_diff_query(self):
        ...
        if ((get_specific_policies == False) or (match_templates != [])):
            ...
            if (match_templates != []):
                # Filter the list of policies and keep only those that are matching. For delete case, playbook policies
                # may either contain template names of policy names. So compare both.
                match_pol = [pl for pl in plist for mt_name in match_templates if (pl['templateName'] == mt_name)]
            else:
                match_pol = plist
                
        if (match_pol):
            # match_pol contains all the policies which exist and match the given templates
            self.changed_dict[0]['query'].extend(list(set([t['templateName'] for t in match_pol if (t['templateName'] not in self.changed_dict[0]['query'])])))
    ...
    def dcnm_policy_create_policy (self, policy, command):
        ...
        while (retries < 3):
            ...
            if (resp.get('DATA', None) != None) and (resp['DATA'].get('failureList', None) != None):
    ...
    def dcnm_policy_send_message_to_dcnm (self):
        ...
        for policy in self.diff_delete:
            ...
            if (isinstance (resp, list)):
                resp = resp[0]
            if (resp and (resp.get('DATA', None) != None) and (resp['RETURN_CODE'] == 200) and resp['MESSAGE'] == 'OK'):
        ...
        for policy in self.diff_create:
            ...
            if (isinstance (resp, list)):
                resp = resp[0]
            if (resp and (resp['RETURN_CODE'] == 200) and (resp['MESSAGE'] == 'OK') and (resp.get('DATA', None) != None)):
                if (resp['DATA'].get('successList', None) != None):
                   if ('is created successfully' in resp['DATA']['successList'][0].get('message')):
                       policy_id = re.findall (r'POLICY-\d+', resp['DATA']['successList'][0].get('message'))
                       if ((self.deploy == True) and (policy_id[0] not in self.deploy_payload)):
        ...
        self.result['changed'] = (mark_delete_flag or saved_flag or delete_flag or create_flag or deploy_flag)
    ...
    def dcnm_translate_switch_info(self, config, ip_sn, hn_sn):
       ...
       for cfg in config:
           ...
           for sw_elem in cfg['switch']:
               addr_info = dcnm_get_ip_addr_info (self.module, sw_elem['ip'], ip_sn, hn_sn)
...
def main():
    ...
    if not dcnm_policy.ip_sn:
        dcnm_policy.result['msg'] = "Fabric {} missing on DCNM or does not have any switches".format(dcnm_policy.fabric)
        module.fail_json(msg="Fabric {} missing on DCNM or does not have any switches".format(dcnm_policy.fabric))

    state = module.params['state']
    if not dcnm_policy.config:
        if state == 'merged' or state == 'deleted' or state == 'query':
            module.fail_json(msg="'config' element is mandatory for state '{}', given = '{}'".format(state, dcnm_policy.config))

    # Convert all hostnames and domain names to IP addresses
    dcnm_policy.dcnm_translate_switch_info (dcnm_policy.config, dcnm_policy.ip_sn,
                                            dcnm_policy.hn_sn)
    ...
    if (dcnm_policy.diff_create or dcnm_policy.diff_modify or dcnm_policy.deploy_payload or dcnm_policy.diff_delete):
